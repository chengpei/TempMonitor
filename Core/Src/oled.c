...
void OLED_Init(void)
{
    ...
    OLED_WriteCmd(0xA4); // display follows RAM

    OLED_WriteCmd(0xD3); // display offset
    OLED_WriteCmd(0x00);

    OLED_WriteCmd(0xD5); // display clock
    OLED_WriteCmd(0x80);
...
{
    uint8_t c = chr - ' ';
    if (c >= sizeof(Font8x16) / 16) return;
...
void OLED_ShowBitmap16x16(uint8_t x, uint8_t y, const uint8_t *bmp)
{
    ...
    for (col = 0; col < 16; col++)
    {
        ...
        {
            /* 上 8 行 */
            line = (bmp[row * 2] << 8) | bmp[row * 2 + 1];
            if (line & (0x8000 >> col))
                upper |= (1 << row);

            /* 下 8 行 */
            line = (bmp[(row + 8) * 2] << 8) | bmp[(row + 8) * 2 + 1];
            if (line & (0x8000 >> col))
                lower |= (1 << row);
        }

        OLED_GRAM[x + col][y]     = upper;
        OLED_GRAM[x + col][y + 1] = lower;
    }
